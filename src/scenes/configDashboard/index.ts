...
import {
  CONFIG_CATEGORIES,
  CONFIG_VARS,
  getConfigVarByEnvKey,
  getConfigVarsByCategory,
  ConfigCategory,
  ConfigVarDefinition,
...
const dbKeysCache = new Set<string>();
let cacheLoaded = false;

async function loadDbKeysCache(): Promise<void> {
  const docs = await database.getAllConfigVars();
  dbKeysCache.clear();
  for (const doc of docs) dbKeysCache.add(doc.key);
  cacheLoaded = true;
}
...
function displayValue(def: ConfigVarDefinition): string {
...
function isFromDB(key: string): boolean {
...
function buildVarDetailKeyboard(def: ConfigVarDefinition) {
  const fromDB = isFromDB(def.envKey);
...
function parseValue(def: ConfigVarDefinition, raw: string): { valid: boolean; error?: string } {
  if (def.type === "number") {
    if (isNaN(Number(raw))) return { valid: false, error: "Must be a number" };
  } else if (def.type === "number[]") {
    const parts = raw.trim().split(/\s+/);
    if (parts.some((p) => isNaN(Number(p)))) return { valid: false, error: "Must be space-separated numbers" };
  }
  return { valid: true };
}

function applyToEnv(def: ConfigVarDefinition, raw: string) {
  let parsed: any;
  switch (def.type) {
    case "number[]":
      parsed = raw.trim().split(/\s+/).map(Number).filter((n: number) => !isNaN(n));
      break;
    case "number":
      parsed = Number(raw);
  ...
  (env as any)[def.envObjKey] = parsed;
}

// Step 0: Show dashboard
const showDashboard = Composer.on("message", async (ctx) => {
  const session = (ctx as any).session as ConfigDashboardSessionData;
  session.currentCategory = null;
  session.editingKey = null;
  session.awaitingInput = false;
  if (!cacheLoaded) await loadDbKeysCache();
...
const waitStep = Composer.on("message", async (ctx) => {
  const wctx = ctx as unknown as ConfigWizardContext;
  const session = wctx.session as ConfigDashboardSessionData;
  ...
  if (!session.awaitingInput || !session.editingKey) return;
  if (!("text" in msg)) return ctx.reply("Please send a text value.");
...
const configDashboard = new Scenes.WizardScene<ConfigWizardContext>(
  "configDashboard",
  showDashboard,
  waitStep
);
...
configDashboard.on("callback_query", async (ctx) => {
  ...
  if (data.startsWith("cfg_cat_")) {
    const category = data.replace("cfg_cat_", "") as ConfigCategory;
    if (!CONFIG_CATEGORIES[category]) return ctx.answerCbQuery("Unknown category");
  ...
  if (data.startsWith("cfg_var_")) {
    ...
    if (!def) return ctx.answerCbQuery("Unknown variable");
    await ctx.answerCbQuery();
    const text = buildVarDetailText(def);
    const keyboard = buildVarDetailKeyboard(def);
  ...
  if (data === "cfg_export_env") {
    try {
      ...
      for (const vDef of CONFIG_VARS) {
        const val = (env as any)[vDef.envObjKey];
