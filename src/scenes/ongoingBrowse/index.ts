...
import OngoingBrowseSessionData from "./sessionData.js";
...
function makeOngoingCaption(channel: any): string {
  return (
...
async function sendChannelPage(
  ...
  const keyboard = buildOngoingPaginationKeyboard(
    page,
    channels.length,
    session.prevCb || "noop",
    session.nextCb || "noop",
    getWatchLink(channel)
  );
...
const ongoingBrowseWizard = new Scenes.WizardScene<WizardContext<OngoingBrowseSessionData>>(
  "ongoingBrowse",
  ...
  Composer.on("message", async (ctx) => {
    ...
    const session = ctx.session as OngoingBrowseSessionData;
      ...
      if (filtered.length === 0) {
        await ctx.reply(
          session.query
            ...
            : "No ongoing dramas available right now.",
          { parse_mode: "HTML" }
        );
