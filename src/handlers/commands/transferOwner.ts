...
export default async function transferOwnerHandler(ctx: CommandContext) {
  ...
  const ready = channels.filter(
    (ch) => ch.pendingOwner && (now - new Date(ch.createdAt).getTime()) >= DAY_MS
  );

  const notReady = channels.filter(
    (ch) => ch.pendingOwner && (now - new Date(ch.createdAt).getTime()) < DAY_MS
  );
  ...
  if (env.twoFaPassword) {
    ...
    for (const ch of ready) {
      try {
        ...
        msg += `❌ ${ch.channelTitle}: ${(error as Error).message}\n`;
 ...
 */
export async function handleTransferOwnerText(ctx: any): Promise<boolean> {
  ...
  for (const { channelId, pendingOwner } of pending) {
    try {
      ...
      msg += `❌ @${pendingOwner}: ${(error as Error).message}\n`;
      logger.error("Ownership transfer failed:", error);
    }
  }
