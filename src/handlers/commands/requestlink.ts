 ...
 */
export default async function requestlinkHandler(ctx: Context): Promise<any> {
  ...
  try {
    ...
    await ctx.reply(
      `✅ <b>Join Request Link Generated!</b>\n\n` +
      `<b>Chat ID:</b> <code>${chatId}</code>\n` +
      `<b>Link:</b> <code>${link}</code>\n\n` +
      `<i>Users clicking this link will need to be approved by the bot/admin.</i>`,
      { parse_mode: "HTML" }
    );
  } catch (err) {
    await ctx.reply(`❌ Failed to create Join Request link: ${(err as Error).message}`);
  }
}
