...
export default async function startHandler(ctx) {
    ...
    try {
            ...
            if (payload.includes("invite") && inviteParts.length > 1) {
                ...
                if (newUserId && inviterId && newUserId !== inviterId) {
                    ...
                    if (!isUserExist) {
                        ...
                        return await sendInviterWelcomeMessage(ctx, inviterId).catch((error) => logger.error("Error sending inviter welcome message:", error));
                    }
                }
            }
            else {
        ...
        if (!shareId) {
            return await sendWelcomeMessage(ctx, user, userId.toString()).catch((error) => logger.error("Error sending welcome message:", error));
        }
        ...
        if (!isValidToken) {
            ...
            if (firstItem && !haveBotPremium) {
                return await sendTokenExpiredMessage(ctx, user, firstItem.sort[0].aioShortUrl, payload).catch((error) => logger.error("Error sending token expired message:", error));
            }
        }
        ...
        if (canRequest || env.adminIds.includes(userId) || haveBotPremium) {
            ...
            return await sendDailyLimitMessage(ctx, user, userId.toString()).catch((error) => logger.error("Error sending daily limit message:", error));
        }
    }
    catch (error) {
...
const addInviteUser = async (inviterId, newUserId, username) => {
