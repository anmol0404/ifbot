 ...
 */
function scheduleAutoDelete(channelId, messageIds, minutes) {
    if (minutes <= 0)
        return;
    ...
    setTimeout(async () => {
        for (const msgId of messageIds) {
            try {
                ...
                logger.error("Auto-delete failed", { channelId, msgId, error: error.message });
            }
        }
 ...
 */
function extractEpisodeInfoRegex(filename, caption) {
 ...
 */
function buildButtonLabel(info, fileSize) {
    const parts = [];
    if (info.episodeNum) {
 ...
 */
async function postFilesLinkMode(channelId, channelTitle, posterID, queuedFiles, chatId, autoDeleteMinutes) {
    ...
    const storedIds = await telegram.forwardMessages(env.dbOngoingChannelId, chatId, messageIds, false, captions);
    ...
    try {
        ...
        logger.error("AI caption generation failed, using fallback", { error: error.message });
    }
 ...
 */
async function postFilesDirectMode(channelId, queuedFiles, chatId, autoDeleteMinutes) {
    const messageIds = queuedFiles.map((f) => f.messageId);
    const captions = queuedFiles.map((f) => f.caption || f.filename);
    // Forward files directly to the target channel
    const postedIds = await telegram.forwardMessages(channelId, chatId, messageIds, false, captions);
 ...
 */
async function processFiles(adminId, files) {
    ...
    if (dbChannels.length === 0) {
        try {
            await telegram.app.telegram.sendMessage(files[0].chatId, "No active ongoing channels registered. Use /ong to add channels first.");
        }
        catch (e) {
    ...
    const channelInfos = dbChannels.map((ch) => ({
    ...
    for (const [channelId, { channel, files: matchedFileRequests }] of matched) {
        ...
        try {
            ...
            await telegram.app.telegram.sendMessage(chatId, `✅ Posted ${queuedFiles.length} file(s) to <b>${channel.channelTitle}</b> (${channel.confidence}% match)\nMode: ${modeLabel}${delLabel}`, { parse_mode: "HTML" });
        }
        catch (error) {
            logger.error("Error posting to channel", { channelId, error: error.message });
            await telegram.app.telegram.sendMessage(chatId, `❌ Failed to post to <b>${channel.channelTitle}</b>: ${error.message}`, { parse_mode: "HTML" });
        }
        await delay(500, 1000);
    }
    ...
    if (unmatched.length > 0) {
        ...
        await telegram.app.telegram.sendMessage(chatId, `⚠️ Could not auto-match ${unmatched.length} file(s):\n<code>${filenames}</code>\n\nSelect a channel to post to:`, {
            parse_mode: "HTML",
...
function suggestChannelName(files) {
    if (files.length === 0)
        return "New Channel";
...
export async function handleUnmatchedCallback(ctx) {
    ...
    if (data === "postto_create") {
        ...
        await ctx.editMessageText(`➕ <b>Create New Channel</b>\n\nSuggested name: <code>${suggested}</code>\n\nSend the channel title (or send the suggested name above):`, { parse_mode: "HTML" });
        return true;
    }
    if (data === "postto_skip") {
    ...
    try {
        ...
        await ctx.editMessageText(`✅ Posted ${unmatchedFiles.length} file(s) to <b>${channel.channelTitle}</b> (${modeLabel})`, { parse_mode: "HTML" });
    }
    catch (error) {
 ...
 */
export async function handleCreateChannelText(ctx) {
    ...
    if (createState.awaitingOwner && createState.channelId && createState.channelTitle) {
        ...
        try {
            if (text !== "/skip") {
                ...
                await ctx.reply(`✅ @${ownerUsername} added as admin.\n` +
                    `Ownership can be transferred after 24h using /transferowner`, { parse_mode: "HTML" });
            }
            // Post files
    ...
    try {
        ...
        await ctx.reply(`✅ Channel created!\n\n` +
            ...
            `👤 Send the <b>username</b> to transfer ownership to (e.g. @username), or /skip:`, { parse_mode: "HTML" });
        // Move to owner step
        ...
        await ctx.reply(`❌ Failed to create channel: ${error.message}`);
        createChannelStore.delete(userId);
        unmatchedStore.delete(userId);
    }
    return true;
