 ...
 */
import { LLMProviderError, LLMTimeoutError, LLMRateLimitError, } from "./types.js";
...
export class CustomAIProvider {
     ...
     */
    async matchFileToChannel(file, channels) {
        if (channels.length === 0)
            return null;
        const startTime = Date.now();
        try {
            const channelList = channels
                .map((ch, i) => `${i + 1}. "${ch.channelTitle}" (keywords: ${ch.keywords.join(", ")})`)
                .join("\n");
...
{ "channelIndex": <number 1-based or 0 if unsure>, "confidence": <0-100>, "reason": "<brief explanation>" }`;
            const result = await this.callAI("You are a file routing assistant for a media bot. Match media files (anime episodes, drama episodes, movies) to their correct series/channel based on filename and caption. Always respond with valid JSON only.", prompt, 3000);
     ...
     */
    async generatePostCaption(channelTitle, files) {
        try {
...
Respond ONLY with the caption text, no JSON, no explanation.`;
            const result = await this.callAIRaw("You are a Telegram channel caption writer. Write short attractive captions for media file posts. Use HTML formatting only.", prompt, 5000);
            return result.trim();
        }
        catch (error) {
     ...
     */
    async extractEpisodeInfo(filename, caption) {
        try {
...
{ "episodeNum": "<episode number or empty>", "quality": "<720p/1080p/480p/etc or empty>", "cleanName": "<clean short episode name>" }`;
            const result = await this.callAI("Extract episode number, quality, and clean name from media filenames. Always respond with valid JSON only.", prompt, 3000);
            return {
     ...
     */
    async callAI(systemPrompt, userPrompt, timeoutMs) {
        const controller = new AbortController();
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new LLMProviderError(`AI server returned ${response.status}`, this.name, response.status, response.status >= 500);
            }
     ...
     */
    async callAIRaw(systemPrompt, userPrompt, timeoutMs) {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), timeoutMs);
        try {
            const response = await fetch(`${this.serverUrl}/v1/chat/completions`, {
