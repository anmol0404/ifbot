...
function formatChannelList(channels) {
    ...
    return channels
        .map((ch, i) => `${i + 1}. <b>${ch.channelTitle}</b>\n` +
...
function settingsKeyboard(channelId, postMode) {
    return {
        inline_keyboard: [
...
function settingsText(title, postMode, autoDeleteMinutes) {
...
function editChannelKeyboard(channelId) {
    return {
        inline_keyboard: [
...
const waitStep = Composer.on("message", async (ctx) => {
    ...
    // === Add channel flow ===
    if (session.action !== "add" || !session.addStep)
        return;
    ...
    if (step === "autoCreateTitle") {
        if ("text" in msg) {
            ...
            try {
                ...
                await ctx.reply(`✅ Channel created: <b>${title}</b>\n` +
        ...
        if ("text" in msg) {
            ...
            if (text !== "/skip") {
                ...
                try {
                    ...
                    await ctx.reply(`⚠️ Could not add @${ownerUsername}: ${error.message}\nContinuing without pending owner.`);
                }
            }
            session.addStep = "keywords";
            await ctx.reply(`Now send <b>keywords</b> for matching, separated by commas (e.g. "one piece, onepiece, op"):`, { parse_mode: "HTML" });
        }
        return;
    }
    if (step === "channelId") {
    ...
    if (step === "poster") {
        ...
        try {
            ...
            await ctx.reply(`✅ Channel added!\n\n` +
                ...
                `Share ID: <code>${shareId}</code>`, { parse_mode: "HTML" });
        }
        catch (error) {
...
const ongDashboard = new Scenes.WizardScene("ongDashboard", showDashboard, waitStep);
...
ongDashboard.on("callback_query", async (ctx) => {
    ...
    if (data === "ong_list") {
        ...
        await ctx.answerCbQuery();
        return ctx.editMessageText(`📋 <b>Registered Channels</b>\n\n${formatChannelList(channels)}`, { parse_mode: "HTML", reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "ong_back" }]] } });
    }
    if (data === "ong_stats") {
        const channels = await database.getAllOngChannels();
        ...
        await ctx.answerCbQuery();
        return ctx.editMessageText(`📊 <b>Stats</b>\n\nTotal Channels: ${channels.length}\nActive: ${totalActive} | Paused: ${totalPaused}\nTotal Episodes Posted: ${totalEps}`, { parse_mode: "HTML", reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "ong_back" }]] } });
    }
    // === Add channel ===
    if (data === "ong_add") {
        session.action = "add";
    ...
    if (data.startsWith("edit_") && !data.startsWith("editf_")) {
        ...
        await ctx.answerCbQuery();
        return ctx.editMessageText(`✏️ <b>${channel.channelTitle}</b>\n\n` +
            ...
            `What do you want to edit?`, { parse_mode: "HTML", reply_markup: editChannelKeyboard(channelId) });
    }
    ...
    if (data === "ong_remove") {
        const channels = await database.getAllOngChannels();
        if (channels.length === 0)
            return ctx.answerCbQuery("No channels to remove");
        const buttons = channels.map((ch) => [
    ...
    if (data.startsWith("settings_")) {
        ...
        if (!channel)
            return ctx.answerCbQuery("Not found");
        await ctx.answerCbQuery();
        return ctx.editMessageText(settingsText(channel.channelTitle, channel.postMode, channel.autoDeleteMinutes), { parse_mode: "HTML", reply_markup: settingsKeyboard(channelId, channel.postMode) });
    }
    if (data.startsWith("setmode_")) {
        ...
        return ctx.editMessageText(settingsText(channel.channelTitle, newMode, channel.autoDeleteMinutes), { parse_mode: "HTML", reply_markup: settingsKeyboard(channelId, newMode) });
    }
    if (data.startsWith("setdel_")) {
        ...
        if (channel) {
            return ctx.editMessageText(settingsText(channel.channelTitle, channel.postMode, minutes), { parse_mode: "HTML", reply_markup: settingsKeyboard(channelId, channel.postMode) });
        }
        return;
    }
    if (data.startsWith("setdelcustom_")) {
        ...
        await ctx.answerCbQuery();
        return ctx.editMessageText("Send custom <b>auto-delete time</b>:\n\n" +
            "<code>30s</code> = 30 seconds\n" +
            "<code>5m</code> = 5 minutes\n" +
            "<code>2h</code> = 2 hours\n" +
            "<code>1d</code> = 1 day\n" +
